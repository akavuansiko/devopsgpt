...
Contexte
Vous avez été recruté(e) par l'entreprise OpenDevOps. Votre première mission est d'aider à concevoir et déployer leur toute nouvelle application web : DevOpsGPT. Ce TP est divisé en 3 parties indépendantes. Si vous bloquez sur une question, passez à la suivante !
...
1. Diagramme de Contexte (2 pts) Faites un schéma simple montrant le système "DevOpsGPT" :
...
2. Organigramme / Flowchart (3 pts) Dessinez l'algorithme (les étapes de fonctionnement) quand un utilisateur envoie un message. Représentez les étapes suivantes avec des blocs et des flèches :
...
3. Dictionnaire de données (1 pt) Faites un tableau listant les informations (attributs) que l'on doit stocker pour un Message. Exemple de colonnes attendues : Nom de la donnée (ex: Contenu), Type (ex: Texte/String), Description. Trouvez au moins 4 attributs pertinents.

Exercice 2 : Git et Docker (7 points)
1. Méthodologie et Git (2 pts) L'équipe utilise la méthode Scrum. On vous demande de développer le système d' "Abonnement Premium".
...
Créer une nouvelle feature nommée feature-premium-subscription et faire un commit.
Une fois le travail terminé et fusionné sur main, créer un tag de version nommé v1.0.0.
Pousser ce tag sur le serveur GitHub.
2. Dockerisation (3 pts) Le projet est divisé en deux dossiers : frontend (l'interface) et backend (l'API). Vous devez créer deux fichiers Dockerfiles :
...
3. Orchestration avec Docker Compose (2 pts) À la racine du projet, créez un fichier docker-compose.yml qui lance :
...
1. Le Workflow CI/CD (5 pts) Créez l'arborescence de dossiers .github/workflows/ et placez-y un fichier main.yml. Écrivez ce fichier YAML en respectant ces consignes :
...
Récupérer le code (utiliser l'action actions/checkout@v4).
Installer Node.js version 22 (utiliser actions/setup-node@v4).
Lancer la commande : npm install.
Lancer la commande : npm test.
Simuler un déploiement avec la commande echo "Déploiement en cours..." UNIQUEMENT si le push est un tag.
2. Sécurité et Secrets (2 pts) Votre code a besoin d'une clé secrète (OPENAI_API_KEY) pour se connecter à l'intelligence artificielle, mais il est strictement interdit de l'écrire en clair dans votre code.
